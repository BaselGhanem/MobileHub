    ...
    <div class="card" style="max-width:520px;margin-top:16px;">
      ...
      <div class="card-body">
        <div style="font-size:13px;color:var(--text2);line-height:2;">
          <div>🔑 Shop ID: <span class="mono" style="color:var(--accent);font-size:12px;">${shopId()||'—'}</span></div>
          <div>📅 تاريخ الإنشاء: ${s.createdAt ? new Date(s.createdAt).toLocaleDateString('ar-JO') : '—'}</div>
          <div>💱 العملة: دينار أردني (JOD)</div>
          <div>🏛️ الضمان: موظف 7.5% + صاحب عمل 14.25%</div>
        </div>
      </div>
    </div>
  `;
  ...
  async save() {
    ...
    try {
      await setDoc(doc(db, COLLECTIONS.SETTINGS, shopId()), {
        shopId: shopId(), name, phone, address, currency: 'JOD',
